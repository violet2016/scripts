        ...
        for query_id, query_info in all_lists.items():
            try:
                ...
                if query_info['end_time'] is not None:
                    ...
                    get_config_sql = 'select cpu_limit, mem_limit, storage_limit from group_configs where group_name = \'%s\' and config_time < \'%s\' order by config_time desc limit 1' % (query_info['groupname'], query_info['start_time'])
