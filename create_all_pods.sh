...
pushd ~/workspace/go-workspace/src/github.com/Pivotal-DataFabric/hawq-misc/hack/docker
sed "s/NUM_GROUP_SIZE/$2/g" Dockerfile-rm-apiproxy-entry.template > Dockerfile-rm-apiproxy-entry
HAWQ_DOCKER_TAG="perf" ./docker_push_test.sh
rm Dockerfile-rm-apiproxy-entry
popd
