...
es2csv -q '*' -u http://localhost:8001/api/v1/namespaces/hawq-monitoring/services/elasticsearch-logging/proxy -i prometheus-2018-09-26 -o ~/workspace/data/hawq-tpch/`date +%Y-%m-%d`/$1/prometheus.csv -k
es2csv -q @'./log.json' -r -u http://localhost:8001/api/v1/namespaces/hawq-monitoring/services/elasticsearch-logging/proxy -i logstash-`date +%Y.%m.%d` -o ~/workspace/data/hawq-tpch/`date +%Y-%m-%d`/$1/api-log.csv -k

rm log.json

#kubectl cp default/hawq-master-667dff4c6c-t8xzq:/data/pivotalhawq/time.txt ~/workspace/data/hawq-tpch/`date +%Y-%m-%d`/$1/
